...
RUN \
  pip3 install -r /opt/double-duty-canvas/docs/requirements.txt \
    && \
...
RUN \
  chgrp --recursive user /opt/double-duty-canvas \
    && \
  chmod --recursive g+rwx /opt/double-duty-canvas \
    && \
  echo "user granted permissions to /opt/double-duty-canvas"

USER user

RUN \
  cd /opt/double-duty-canvas \
    && \
...
WORKDIR /opt/double-duty-canvas
